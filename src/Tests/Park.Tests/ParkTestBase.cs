...
namespace Park.Tests
{
    public abstract class ParkTestBase : AbpIntegratedTestBase<ParkTestModule>
    {
        ...
        protected void UsingDbContext(Action<ParkDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ParkDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ParkDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ParkDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ParkDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ParkDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ParkDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ParkDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ParkDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ParkDbContext, Task<T>> func)
        {
            T result;
