...
CustomerCarUserGroupMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<CarUserGroupAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<CarUserGroup>  CarUserGroups { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new CarUserGroupCfg());
        }
