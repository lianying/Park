...
CustomerCarTypesMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<CarTypesAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<CarTypes>  CarTypess { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new CarTypesCfg());
        }
