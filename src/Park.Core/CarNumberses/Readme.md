...
CustomerCarNumbersMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<CarNumbersAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<CarNumbers>  CarNumberss { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new CarNumbersCfg());
        }
