...
CustomerParkEntrancesMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<ParkEntrancesAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<ParkEntrances>  ParkEntrancess { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new ParkEntrancesCfg());
        }
