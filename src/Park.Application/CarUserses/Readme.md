...
CustomerCarUsersMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<CarUsersAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<CarUsers>  CarUserss { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new CarUsersCfg());
        }
