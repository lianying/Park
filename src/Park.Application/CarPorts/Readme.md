...
CustomerCarPortMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<CarPortAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<CarPort>  CarPorts { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new CarPortCfg());
        }
