...
CustomerParkAreasMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<ParkAreasAppAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<ParkAreas>  ParkAreass { get; set; }
...
```
        ...
        {
        modelBuilder.ApplyConfiguration(new ParkAreasCfg());
        }
